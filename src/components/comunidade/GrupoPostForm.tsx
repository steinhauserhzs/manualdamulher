...
export const GrupoPostForm = ({ grupoId, onSuccess }: GrupoPostFormProps) => {
    ...
    try {
      ...
      const { data: post, error: postError } = await supabase
        ...
        .insert({
          ...
          grupo_id: grupoId, // Associar ao grupo
        })
        .select()
        .single();
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <RadioGroup value={tipo} onValueChange={setTipo} className="flex flex-wrap gap-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="pergunta" className="cursor-pointer">❓ Pergunta</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="dica" id="dica" />
            <Label htmlFor="dica" className="cursor-pointer">💡 Dica</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="enquete" id="enquete" />
            <Label htmlFor="enquete" className="cursor-pointer">📊 Enquete</Label>
          </div>
        </RadioGroup>
      </div>
      ...
      <div>
        ...
        {imagemPreview && (
          <div className="relative mt-2">
            <img src={imagemPreview} alt="Preview" className="rounded-lg max-h-32 w-full object-cover" />
            <Button
              type="button"
              variant="destructive"
              size="icon"
      ...
      {tipo === "enquete" && (
        <div className="space-y-2">
          ...
          {opcoesEnquete.map((opcao, index) => (
            <div key={index} className="flex gap-2">
              ...
              {opcoesEnquete.length > 2 && (
                <Button type="button" variant="ghost" size="icon" onClick={() => removerOpcaoEnquete(index)}>
                  <X className="h-4 w-4" />
                </Button>
              )}
            </div>
          ...
          {opcoesEnquete.length < 5 && (
            <Button type="button" variant="outline" size="sm" onClick={adicionarOpcaoEnquete}>
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Input
            ...
            onKeyPress={(e) => { if (e.key === "Enter") { e.preventDefault(); adicionarTag(); } }}
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag, index) => (
              <Badge key={index} variant="secondary" className="flex items-center gap-1">
                #{tag}
                <button type="button" onClick={() => removerTag(index)} className="ml-1 hover:text-destructive">
                  <X className="h-3 w-3" />
                </button>
              </Badge>
            ))}
      ...
      <Button type="submit" className="w-full" disabled={loading}>
        {loading ? "Publicando..." : "Publicar no Grupo"}
      </Button>
    </form>
  );
};
